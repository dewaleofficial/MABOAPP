...
import { Money, MinorUnits, CurrencyCode, minorUnits, money } from '@provia/types';
...
export const scale = (m: Money, multiplier: number): Money => {
  if (!Number.isFinite(multiplier) || multiplier < 0) {
    throw new TypeError(`Multiplier must be a finite non-negative number, received ${multiplier}`);
  }
  return {
...
export const discountPercent = (m: Money, percent: number): Money => {
  if (percent < 0 || percent > 100) {
    throw new RangeError(`Discount percent must be between 0 and 100, received ${percent}`);
  }
...
export const allocate = (m: Money, ratios: readonly number[]): Money[] => {
  ...
  const parts = ratios.map((r) => Math.floor((m.amount * r) / totalRatio));
...
export const fromMajor = (major: number, currency: CurrencyCode): Money => {
  ...
  if (!Number.isInteger(Math.round(minor * 1000) / 1000) || !Number.isInteger(Math.round(minor))) {
    throw new TypeError(`${major} ${currency} has more precision than the currency supports`);
  }
