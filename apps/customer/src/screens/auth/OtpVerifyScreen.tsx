 ...
 *
 * Step 2 of sign-in. verifyOtp is the call that actually creates the
 * Supabase session — this is the exact JWT the backend's AuthGuard
 * verifies (CLAUDE.md §9). Once this succeeds, supabase.auth.getSession()
 * returns a real, signed token for every subsequent API call.
 */
...
export function OtpVerifyScreen({ route }: Props) {
  ...
  // No navigation.navigate() call on success — RootNavigator switches
  // from AuthStack to the main app stack automatically once
  // onAuthStateChange fires with a real session (see navigation/
  // RootNavigator.tsx). This screen's only job is to make the real call.
